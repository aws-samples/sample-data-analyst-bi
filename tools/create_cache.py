...
import json
import psycopg2
from config import vector_db_config, vector_table_name
...
def create_vector_cache(cur):
    ...
    """
    try:
        print(f"Table Name: {vector_table_name}")
        cur.execute(f"""
            ...
            WITH (lists = 100);
        """)
        print(f"Table '{vector_table_name}' created successfully with vector index.")
    except psycopg2.Error as e:
